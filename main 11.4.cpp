...
import pandas as pd
    ...
    def check_header(file_path, expected_columns) :
    try :
...
def main() :
    file_path = input("请输入表格文件路径：")
    expected_columns = ["选课时间", "学号", "姓名", "课程名称", "学分", "百分成绩", "五分成绩", "考试类型", "选修类型"]

    if check_header(file_path, expected_columns) :
        print("表头包含了所有指定的列名。")
    else:
print("表头缺少指定的列名。")
